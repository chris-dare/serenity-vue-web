      ...
      <div class="space-y-4">
        ...
        <div>
          ...
          >
            ...
            <div class="flex mb-2">
              <div class="w-1/4">
                <div>{{ row.drug.medication_detail[0].display }}</div>
              </div>
              <div class="w-1/4">
                <div>{{ row.drug.duration }}</div>
              </div>
              <div class="w-1/4">
                <div>{{ row.drug.dosage }}</div>
              </div>
              <div class="w-1/4">
                <div>{{ row.drug.instruction }}</div>
              </div>
            </div>
            <div v-if="row.filled">
            ...
            <div class="flex items-center justify-end mt-4">
              <SeButton
                :variant="row.filled == true ? 'secondary' : 'secondary-outline'"
                class="mr-4"
                @click="row.filled = true"
              >
                Prescription filled<Checkmark class="w-4 h-4 text-white ml-4" />
              </SeButton>
              <SeButton
                :variant="row.filled == true ? 'danger-outline' : 'danger'"
                @click="row.filled = false"
              >
