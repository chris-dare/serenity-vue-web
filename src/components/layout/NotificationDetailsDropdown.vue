<template>
  <Dropdown :visible.sync="visible" with-shadow>
    ...
    <div class="w-96 bg-white">
      <div class="flex border-b border-solid border-subtle p-4" v-for="(option, index) in options" :key="index">
        <div class="flex justify-center items-center rounded-full h-9 w-9 mr-3" :class="option.color">
            <component :is="option.component" class="w-5 h-5 text-white" />
        </div>
        <div>
            <div class="text-xs">
